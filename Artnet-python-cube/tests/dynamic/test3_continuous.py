...
filename = "tests/logs/dynamic/test3_continuous.txt"
file = open(filename, "a")


def main_no_artsync(universe1, universe2, ip1='127.0.0.1', ip2='127.0.0.1', tmp=0.5, pause=5):
    packet_size = 512
    port = 6454
...
def main_artsync(universe1, universe2, ip1='127.0.0.1', ip2='127.0.0.1', tmp=0.5, pause=5):
    packet_size = 512
    port = 6454
    a1 = StupidArtnet(ip1, port, universe1, packet_size)
    a2 = StupidArtnet(ip2, port, universe2, packet_size)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        if type == "noartsync":
            main_no_artsync(int(universe1), int(universe2), ip1, ip2, float(time), float(pause_time))
        elif type == "artsync":
            main_artsync(int(universe1), int(universe2), ip1, ip2, float(time), float(pause_time))
        file.write(f"test ending: {datetime.datetime.now()}\n")
    else:
        print("Wrong arguments,arguments: type , ip1, ip2, universe1, universe2, animation_time, pause time")
